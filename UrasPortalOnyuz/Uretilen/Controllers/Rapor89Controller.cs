// <ornek-uretildi/> Bu dosya ana projeden OTOMATIK uretildi: govdeler sokuldu, baglanti yok.
#pragma warning disable
using Microsoft.AspNetCore.Mvc.Rendering;
using System.Linq;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.Logging;
using System;
using System.Collections.Concurrent;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Threading.Tasks;

namespace WebApplication3.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class Rapor89Controller : Controller
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<Rapor89Controller> _logger;


        private readonly List<string> _resmiSirketler = new List<string>
        {
            "ALV_KIMYA", "AVRUPA_PAPER", "URSMAKINE", "ASIA_KIMYA"
        };

        private readonly List<DatabaseConfig> _databases = new List<DatabaseConfig>
        {
            new DatabaseConfig { Key = "DefaultConnection", Display = "URASKIMYA" },
            new DatabaseConfig { Key = "DefaultConnection1", Display = "URSMAKINE" },
            new DatabaseConfig { Key = "DefaultConnection3", Display = "ALV_KIMYA" },
            new DatabaseConfig { Key = "DefaultConnection4", Display = "DAF_KIMYA" },
            new DatabaseConfig { Key = "DefaultConnection5", Display = "SELVI" },
            new DatabaseConfig { Key = "DefaultConnection6", Display = "AVRUPA_PAPER" },
            new DatabaseConfig { Key = "DefaultConnection7", Display = "ALVFILO" },
            new DatabaseConfig { Key = "DefaultConnection8", Display = "ASIA_KIMYA" },
            new DatabaseConfig { Key = "DefaultConnection9", Display = "DEKORLIM" },
            new DatabaseConfig { Key = "DefaultConnection10", Display = "URAS_HOLDING" },
            new DatabaseConfig { Key = "DefaultConnection11", Display = "DRN" },
            new DatabaseConfig { Key = "DefaultConnection32", Display = "URASHOLDING_2026" },
            new DatabaseConfig { Key = "DefaultConnection33", Display = "DAFKIMYA_2026" },
            new DatabaseConfig { Key = "DefaultConnection34", Display = "AVRUPAPAPER_2026" },
            new DatabaseConfig { Key = "DefaultConnection35", Display = "ALVFILO_2026" },
            new DatabaseConfig { Key = "DefaultConnection36", Display = "URASBASKI_2026" },

            new DatabaseConfig { Key = "DefaultConnection2", Display = "AVRUPA_PAPER" },
            new DatabaseConfig { Key = "DefaultConnection14", Display = "ALVFILO_AS" },
            new DatabaseConfig { Key = "DefaultConnection15", Display = "AVRUPAPAPER_AS" },
            new DatabaseConfig { Key = "DefaultConnection16", Display = "DAFKIMYA_AS" },
            new DatabaseConfig { Key = "DefaultConnection17", Display = "SELVI_A.S" },
            new DatabaseConfig { Key = "DefaultConnection18", Display = "URASHOLDING_A.S" },
            new DatabaseConfig { Key = "DefaultConnection19", Display = "DRN" },
            new DatabaseConfig { Key = "DefaultConnection20", Display = "ALVKIMYA_2026" },
            new DatabaseConfig { Key = "DefaultConnection21", Display = "URSMAKINE_2026" },
            new DatabaseConfig { Key = "DefaultConnection22", Display = "SELVI_2026" },
            new DatabaseConfig { Key = "DefaultConnection23", Display = "TESTURASKIMYA_A.SS" },
            new DatabaseConfig { Key = "DefaultConnection24", Display = "TESTSELVI" },
            new DatabaseConfig { Key = "DefaultConnection25", Display = "URASKIMYA_AS" },
            new DatabaseConfig { Key = "DefaultConnection26", Display = "URAS_BASKI" },
            new DatabaseConfig { Key = "DefaultConnection27", Display = "DELTA_POWER" },
            new DatabaseConfig { Key = "DefaultConnection28", Display = "MORAL_POWER" },
            new DatabaseConfig { Key = "DefaultConnection29", Display = "SADE_POWER" },
            new DatabaseConfig { Key = "DefaultConnection30", Display = "URAS_POWER" },
        };

        [HttpGet]
        [HttpGet("Index")]
        public IActionResult Index()
 {WebApplication3.OrnekDoldurucu.Hazirla(this);
return View("Index");
}

        [HttpGet("GetFiltersData")]
        public async Task<IActionResult> GetFiltersData()
 {return Json(new { success = true, companies = global::System.Linq.Enumerable.Range(0, 12).Select(i2 => global::WebApplication3.OrnekDoldurucu.Deger<string>("companies", i2)).ToList(), depolar = global::System.Linq.Enumerable.Range(0, 12).Select(i2 => new { Value = global::WebApplication3.OrnekDoldurucu.Deger<string>("Value", i2), Text = global::WebApplication3.OrnekDoldurucu.Deger<string>("Text", i2) }).ToList(), stokGruplari = global::System.Linq.Enumerable.Range(0, 12).Select(i2 => new { Value = global::WebApplication3.OrnekDoldurucu.Deger<string>("Value", i2), Text = global::WebApplication3.OrnekDoldurucu.Deger<string>("Text", i2) }).ToList() });
}

        [HttpPost("GetReportData")]
        public async Task<IActionResult> GetReportData([FromBody] Rapor89FilterModel filter)
 {return Json(new { success = true, summary = global::WebApplication3.OrnekDoldurucu.Yeni<global::WebApplication3.Controllers.Rapor89Summary>(), data = global::WebApplication3.OrnekDoldurucu.Liste<global::WebApplication3.Controllers.Rapor89Dto>(12) });
}
    }

   

    public class Rapor89FilterModel
    {
        public List<string> Companies { get; set; }
        public List<string> Depolar { get; set; }
        public List<string> StokGruplari { get; set; }
        public string StokTipi { get; set; }
        public string KontrolAyi { get; set; }
    }

    public class Rapor89Summary
    {
        public int ToplamStokKalemi { get; set; }
        public decimal FiiliStokBedeli { get; set; }
        public decimal MuhasebeStokBedeli { get; set; }
        public decimal BedelFarki { get; set; }
        public int AylikUyumsuzSayisi { get; set; }
        public int AylikUyumluSayisi { get; set; }
        public int UcluUyumsuzSayisi { get; set; }
        public int UcluUyumluSayisi { get; set; }
    }

    public class Rapor89Dto
    {
        public string Sirket { get; set; }
        public string Depo { get; set; }
        public string DepoAdi { get; set; }
        public string StokTipi { get; set; }
        public string UrunGrubu { get; set; }
        public string UrunKodu { get; set; }
        public string UrunAdi { get; set; }
        public string Birim { get; set; }
        public decimal BirimMaliyet { get; set; }
        public decimal MuhasebeStokMiktari { get; set; }
        public decimal MuhasebeStokBedeli { get; set; }
        public decimal FiiliStokMiktari { get; set; }
        public decimal FiiliStokBedeli { get; set; }
        public decimal MiktarFarki { get; set; }
        public decimal BedelFarki { get; set; }
        public string AylikUyumDurumu { get; set; }
        public string UcluUyumDurumu { get; set; }
    }
}